...
if __name__ == "__main__":
    ...
    #roi = np.array([[62, 39], [185, 146]])   #car


    roi = np.array([[260, 60], [315, 155]])   # Bolt
    rect_tl_pt = np.array([roi[0][0], roi[0][1], 1])
    rect_br_pt = np.array([roi[1][0], roi[1][1], 1])
    frame = 2

    fourcc = cv2.VideoWriter_fourcc(*'XVID')
    out = cv2.VideoWriter("track_Bolt.avi", fourcc, 10.0, (width,height))
    ...
    while True:
        ...
        cv2.rectangle(image_copy, tuple(rect_tl_pt_new), tuple(rect_br_pt_new), (0, 0, 255), 1)
